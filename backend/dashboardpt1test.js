...
ORDER BY energy_meter_id;
  ...
  const validZones = meterWiseConsumption.filter(zone => zone.consumption > 0);

  const highZone = validZones.length > 0
    ? validZones.reduce((prev, curr) => (curr.consumption > prev.consumption ? curr : prev))
  ...
  console.log("✅ fetchConsumptionMeterWise result:", { meterWiseConsumption, hlCons });
